...
class Article extends Model
{
    ...
    {
        //Cache::forget(self::REDIS_ARTICLE_CACHE . $articleId);
        //if (empty($article = Cache::get(self::REDIS_ARTICLE_CACHE . $articleId))) {
            $article = self::find($articleId);
            if($article){
                //获取标签
                $article->tags = Tag::getTagModelByTagIds($article->tags);
                //获取点击次数
                $article->click = ArticleStatus::getArticleStatusbyArticleid($article->id)->view_number;
            }
